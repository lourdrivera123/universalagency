						...
						<div class="widget-body no-padding">
							...
							<table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
								...
								<tbody>
									...
									<tr data-jobid="{{ $pendingjobrequest->job_id }}">
										<td>{{ $pendingjobrequest->id }}</td>
										<td>	<a href="{{ URL::to('job/'.$pendingjobrequest->job()->first()->id ) }}" target="_blank"> {{ $pendingjobrequest->job()->first()->job_title }} </a> </td>
										<td><span data-toggle="modal" data-target="#listrequestingapplicants" onclick="listrequestingapplicants($(this))" class="badge pull-right inbox-badge bg-color-blue" style="padding:12px;cursor:hand"> {{ countpendingjobrequests($pendingjobrequest->job_id).' applicant/s requesting for this job' }}</span> </td>
									</tr>
									@endforeach
								</tbody>
							</table>
...
<div class="modal fade" id="listrequestingapplicants" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true">
				...
				<table class="table table-striped table-bordered table-hover" width="100%">
					<thead>
						<tr>
							...
							<th colspan="2"><i class="fa fa-fw fa-user text-muted hidden-md hidden-sm hidden-xs" ></i> Action </th>
						</tr>
					</thead>
